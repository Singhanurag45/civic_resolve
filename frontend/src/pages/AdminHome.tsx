...
const AdminHome = () => {
    ...
    const fetchIssues = async () => {
      try {
        const response = await fetch(
          `${import.meta.env.VITE_API_BASE_URL}/api/v1/all-issues`,
          {
            headers: {
  ...
  const handleStatusUpdate = async (issueId: string, status: string) => {
    try {
      const response = await fetch(
        ...
        }/api/v1/admin/issue/${issueId}/status`,
        {
          method: "PUT",
          headers: {
  ...
  const handleDeleteIssue = async (issueId: string) => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_BASE_URL}/api/v1/issue/admin/${issueId}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${localStorage.getItem("auth_token")}`,
      ...
      <div className="min-h-screen bg-[#f3f6f8]">
        ...
        <div className="pt-20 container mx-auto px-4 py-8 space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 ">
            <div className="p-6 rounded-lg border shadow-lg bg-card hover:scale-[1.02] hover:shadow-xl transition-all duration-300 ">
