...
const Header: React.FC<HeaderProps> = ({
  ...
  const isDashboardRoute = location.pathname.startsWith("/admin") || location.pathname.startsWith("/citizen");
          ...
          <div className="flex items-center space-x-3">
            {user ? (
              <>
                ...
                <Link to={user.role === "citizen" ? "/citizen" : "/admin"} aria-current={isDashboardRoute ? "page" : undefined}>
                  <Button
                    variant={isDashboardRoute ? "default" : "outline"}
                    size="sm"
                    className={`flex items-center space-x-2 ${
                      isDashboardRoute
                        ? "civic-gradient text-white border-0 shadow-md"
                        : ""
                    }`}
                  >
